...
public:
    ...
    MatriXXX_Destructor();// Destructor that memory free.

    double Get_Element(unsigned int row,unsigned int col) const;// Returns the value of the matrix element. Parameters: row, column

    void Set_Element(unsigned int row, unsigned int col,
                     double value);//Assigns a value to the matrix element. Parameters: row, column, value assigned

    void MatriXXX_print();//Displays a matrix on the screen.
    ...
    extern MatriXXX MatriXXX_zero(const unsigned int rows, const unsigned int cols);//Zero Matrix
    ...
    extern MatriXXX MatriXXX_one(unsigned int rows, unsigned int cols);
